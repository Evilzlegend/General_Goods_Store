...
app.get('/getAllAppliances', async (req, res) => {
    try {
        let appliances = await Appliance.find({}).sort({ "applianceBrand": 1 }).lean();
...
app.get('/getAllElectronics', async (req, res) => {
    try {
        let electronics = await Electronics.find({}).sort({ "electronicsBrand": 1 }).lean();
...
app.get('/getAllHardware', async (req, res) => {
    try {
        let hardwares = await Hardware.find({}).sort({ "hardwareBrand": 1 }).lean();
...
//edit -----------------------------------------------
app.post('/editHardwareById', async (req, res) => {
    try {
...
app.get('/getAllHomegoods', async (req, res) => {
    try {
        let homegoods = await Homegoods.find({}).sort({ "homegoodsBrand": 1 }).lean();
...
app.get('/getAllPerishables', async (req, res) => {
    try {
        let perishables = await Perishables.find({}).sort({ "perishableBrand": 1 }).lean();
